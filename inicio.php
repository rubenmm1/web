...
<body class="main-body">
	...
	<div class="page">
		...
		<div class="main-content horizontal-content">
			...
			<div class="container">
					...
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">Home</h4><span
								class="text-muted mt-1 tx-13 ms-2 mb-0">/ Máquinas</span>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
              ...
              where activa=1 and id_cliente =" . $_SESSION['cod_cliente'];
					...
					if ($result->num_rows == 0) { // Usuario web no existente
						...
						while ($fila1 = $result->fetch_assoc()) {
							if (strlen($fila1['descripcion_maquina']) > 35) {
								$descripcion = substr($fila1['descripcion_maquina'], 0, 35).'...';
							}else{
								$descripcion = $fila1['descripcion_maquina'];
							}
							echo '<div class="col-12 col-sm-6 col-lg-6 col-xl-3">
										<div class="card card-primary">
											<div class="card-header pb-0">
												<h5 class="card-title mb-0 pb-0">' . $fila1['nombre'] . '</h5>
												<small>SN: <strong>' . $fila1['nserie'] . '</strong></small>
												<img src="fotos/' . $fila1['cod_articulo'] . '/' . $fila1['cod_articulo'] . '.jpg" class="img-fluid" />
											</div>
											<div class="card-body text-dark">
											<h5>' . $fila1['nombre_maquina'] . '</h5>
												' . $descripcion . '
											</div>
											<div class="card-footer text-right">
												<small><div class="btn btn-success btn-sx btn_ampliar_ficha" cod="' . $fila1['cod_maquina'] . '" >Ampliar Ficha</div></small>
											</div>
										</div>
									</div>';
						}
				...
				<form id="formulario_ampliar_ficha" method="POST" action="maquina.php">
					<input type="hidden" id="txt_cod_maquina" name="txt_cod_maquina" />
				</form>
			</div>
			<!-- Container closed -->
		</div>
	...
	<script>
		$(".btn_ampliar_ficha").on("click", function () {
			$("#txt_cod_maquina").val($(this).attr("cod"))
			$("#formulario_ampliar_ficha").submit();
		})
	</script>
</body>
